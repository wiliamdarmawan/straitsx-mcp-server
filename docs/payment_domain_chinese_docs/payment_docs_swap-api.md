...
# https://docs.straitsx.com/docs/swap-api
...
![](https://files.readme.io/87c446c-image.png)

##

Supported Swap Pairs and FX Quote

In order to perform a swap, you first need to get the swap pairs currently
supported by StraitsX via the [get supported swap pairs](ref:get-supported-
swap-pairs) 接口端点. For example, if you receive `XSGDUSDC` in the 响应,
that means you will be allowed to swap between XSGD and USDC in both
directions.
...
Upon confirming the supported swap pairs, call the [请求 swap quote
接口端点](ref:create-a-swap-quote) to get the rate for the 货币 pair,
金额, and fixed side. Note that each quote returned has an expiry time.

##

Execute a quote

While the quote is still valid, [execute the swap quote](ref:execute-a-swap-
quote) with the quote id returned earlier. Once the swap is executed, the
balance for the respective wallets will be updated.

__Updated about 1 month ago

* * *

[区块链 转账 Out API](/docs/区块链-转账-out-guide)[Purpose
Code for FAST Payouts](/docs/purpose-code-for-FAST-payouts)
