...
# https://docs.straitsx.com/docs/first-party-转账
...
# First Party 转账 (客户 Profile& 客户 Profile+)

This integration type is intended for partners who exclusively use StraitsX
API to collect 付款 from their users and make payouts to their users'
external 银行 accounts. We will require information on your users as per
regulations and all incoming and outgoing transactions are restricted to first
party only, i.e. your user must deposit from and withdraw to a 银行 account in
their name.

Some examples of partners under this model are crypto exchanges. If you are
unsure whether you fall under this category, [contact us](doc:support).
...
Learn about the difference between CP and CP+ accounts.
